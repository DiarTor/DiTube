...
class UserAdministration:
    ...
    def include_user_balance(self, msg: telebot.types.Message, bot: telebot.TeleBot):
        ...
        try:
            ...
            bot.send_message(msg.chat.id,
                             "❌ لطفا دستور را به این صورت وارد کنید:\n /include_balance [ایدی عددی کاربر] [اعتبار] [پیام]")
    ...
    def get_user_stat(self, msg: telebot.types.Message, bot: telebot.TeleBot):
        ...
        try:
            ...
            user_stats = {"username": user["user_name"], "user_id": user["user_id"],
                ...
                "referral_total_profit": user["referral_total_profit"], "subscription_type": subscription_type,
                "subscription_days": subscription_days, "subscription_days_left": subscription_days_left, }

            response = persian.get_user_stat_message.format(user_stats["username"], user_stats["user_id"],
                user_stats["register_date"], user_stats['downloads'], user_stats["downloads_size"],
                user_stats["balance"], user_stats["total_money_charged"], user_stats["referrals"],
                user_stats["referral_total_profit"], user_stats["subscription_type"], user_stats["subscription_days"],
    ...
    def send_message_to_user(self, msg: telebot.types.Message, bot: telebot.TeleBot):
        ...
        try:
            ...
            bot.send_message(msg.chat.id, "❌ لطفا دستور را به این صورت وارد کنید:\n /send_message_to_user [ایدی عددی کاربر] [پیام]")
